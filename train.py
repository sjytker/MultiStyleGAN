...
train_display_s_cloth = torch.stack([train_loader_s_cloth.dataset[i] for i in range(display_size)]).cuda()
train_display_d_water = torch.stack([train_loader_d_water.dataset[i] for i in range(display_size)]).cuda()
train_display_s_water = torch.stack([train_loader_s_water.dataset[i] for i in range(display_size)]).cuda()
...
while True:
    ...
    with Timer("Elapsed time in update: %f"):
        # Main training code
        trainer.dis_update(sc, dw, sw, config)
        trainer.gen_update(sc, dw, sw, config)
